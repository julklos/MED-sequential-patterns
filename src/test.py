...
class PrefixSpan:
	...
	def __init__(self, sequences, minSupport=0.1, maxPatternLength=10):
		...
		freqSequences = self._prefixSpan(
			SequencePattern([], None, maxPatternLength, self.PLACE_HOLDER), 
			sequences, minSupport, maxPatternLength)

		self.freqSeqs = FreqSequences(freqSequences)
	...
	def _prefixSpan(self,pattern, S, threshold, maxPatternLength):
		...
		if self._checkPatternLengths(pattern, maxPatternLength):
			...
			for i in f_list:
				p = SequencePattern(pattern.sequence, pattern.freq, maxPatternLength, self.PLACE_HOLDER)
				p.append(i)
				if self._checkPatternLengths(pattern, maxPatternLength):
					patterns.append(p)
	...
	def _frequent_items(self, S, pattern, threshold, maxPatternLength):
		...
		f_list.extend([SequencePattern([[self.PLACE_HOLDER, k]], _items[k], maxPatternLength, self.PLACE_HOLDER)
				   for k in _items
				   if _items[k] >= threshold])
		f_list.extend([SequencePattern([[k]], items[k], maxPatternLength, self.PLACE_HOLDER)
				   for k in items
